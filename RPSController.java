...
public class RPSController implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		String winner = "";
		this.model.playRound(player); // Play Hand
		this.view.setPlayerLabel(player); // Display users hand
		x = this.model.getComputerHand();

		this.view.setComputerLabel(x); // Display computers hand
		if (this.model.computerWins()) // Determines Winner
		{
			winner = RPSUtils.COMPUTER;
		} else if (this.model.playerWins()) {
			winner = RPSUtils.PLAYER;
		} else {
			winner = RPSUtils.DRAW;
		}
		this.view.setWinner(winner); // Sets Winner
		this.view.setRecord(this.model.getRoundsWonByPlayer(), this.model.getRoundsWonByComputer(),
				this.model.getRoundsDrawn());
	}
