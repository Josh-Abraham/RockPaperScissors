...
public class RPSModel {
	...
	public void playRound(String player) {
		...
		if (!(this.player.equals(RPSUtils.ROCK) || this.player.equals(RPSUtils.PAPER)
				|| this.player.equals(RPSUtils.SCISSORS))) {
			throw new IllegalArgumentException();
		} else if (playerWins()) {
			this.roundsWonByPlayer = getRoundsWonByPlayer() + 1;
		} else if (computerWins()) {
			this.roundsWonByComputer = getRoundsWonByComputer() + 1;
		} else {
			this.roundsDrawn = getRoundsDrawn() + 1;
		}

		this.roundsPlayed = getRoundsPlayed() + 1;
	...
	public boolean computerWins() {
		...
		if ((this.computer.equals(RPSUtils.ROCK) && this.player.equals(RPSUtils.SCISSORS))) {
			equals = true;
		} else if ((this.computer.equals(RPSUtils.SCISSORS) && this.player.equals(RPSUtils.PAPER))) {
			equals = true;

		} else if ((this.computer.equals(RPSUtils.PAPER) && this.player.equals(RPSUtils.ROCK))) {
			equals = true;
		}
		return equals;
	...
	public boolean playerWins() {
		boolean equals = false;
		if ((this.player.equals(RPSUtils.ROCK) && this.computer.equals(RPSUtils.SCISSORS))) {
			equals = true;
		} else if ((this.player.equals(RPSUtils.SCISSORS) && this.computer.equals(RPSUtils.PAPER))) {
			equals = true;

		} else if ((this.player.equals(RPSUtils.PAPER) && this.computer.equals(RPSUtils.ROCK))) {
			equals = true;
		}
		return equals;
	}
